 ...
 *
 * Reads DD XLSX reference sheets (Fields + Lookups tabs) for each version,
...
const OUTPUT_DIR = join(__dirname, 'dd-output');
...
/**
 * Reads an XLSX sheet and returns an array of row objects.
 * All values are coerced to trimmed strings (matching prior CSV behavior).
 * Excel date serial numbers are converted to M/D/YYYY format.
 */
const DATE_COLUMNS = new Set(['StatusChangeDate', 'RevisedDate']);
  ...
  return XLSX.utils.sheet_to_json(sheet, { defval: '' }).map(row =>
    Object.fromEntries(
      Object.entries(row).map(([k, v]) => {
        const key = k.trim();
        if (DATE_COLUMNS.has(key) && typeof v === 'number') {
...
const loadVersion = version => {
  ...
  const fields = readSheet(workbook, 'Fields');
  const lookups = readSheet(workbook, 'Lookups');
